...
chat_prompt = ChatPromptTemplate.from_messages([
    SystemMessagePromptTemplate.from_template(
        "당신은 기업의 커뮤니케이션 전문가 입니다."
        "짧고 포멀하게 전문가 톤으로 이메일을 작성하시오"),
...
topics = [
    ...
    "분기별 개별 성과 지표",
    "개인별 매출 목표치 달성 현황 리뷰",
    "개발을 못해서 맨날 버그만 발생시키는 개발자 해고"]

for recipient, topic in zip(recipients,topics):
    print("--------")
    print(f"TO: {recipients}")
    print("--------")
    result = chain.invoke({"recipient": recipients, "topic": topic})
    print(result)
    print("--------")

# result = chain.invoke({"recipient": recipients[0], "topic": topic[0]})
# print(result)