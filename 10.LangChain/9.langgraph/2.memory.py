...
result1 = app.invoke({"messages": [HumanMessage(content=first_input)]}, config=config)
second_input = "제 이름이 뭐였죠?"
result2 = app.invoke({"messages": [HumanMessage(content=second_input)]}, config=config)
...
first_input = "저는 프로그래머입니다.."
result3 = app.invoke({"messages": [HumanMessage(content=first_input)]}, config=config)
second_input = "제 이름은 무엇이고, 저는 무슨일을 하나요??"
result4 = app.invoke({"messages": [HumanMessage(content=second_input)]}, config=config)
