...
def build_docment():
    chunks = []
    for path in FILES:
        paths = splitter.split_documents(load_any_docs(path))
        for c in paths:
            c.metadata("source") = os.path.basename(path) # 통합된 컬렉션 내에서 문서를 구분하기위해, 각각의 청크에 메타데이터를 넣음
        chunks += paths
