...
def build_docment(file_path,collection):
...
# 1. 컬레션 두개 준비
collections = {
    "nvme": build_docment("./nvme.txt","nvme"),
    "hbm": build_docment("./hbm.txt","hbm"),
}

for name, store in collections.items():
    print(f"컬렉션: {name}, 청크갯수 : {store._collection.count()}")
...
print("\n === 'nvme, hbm' 등이 있는 컬렉션에 이걸 다 물어보려면?")
