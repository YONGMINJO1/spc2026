...
CREATE TABLE products (in INTEGER PRIMARY KEY, name TEXT, price INTEGER, category TEXT);
...
@tool
def run_sql(querey:str) -> str:
    """sQLite DB에 SQL 구문을 실행하고 결과를 반환한다."""
    q = querey.strip().rsplit(";")
    cur = conn.execute(q)
    ...
    # 위에 쿼리 결과를 최대한 이쁘게 지연어로 변환하는 코드
    out = [" | ".join(cols)]
    out += [" | ".join(str(v) for v in row) for row in rows]
    return "\n".join(out)

SYSTEM =f"""
당신은 SQL 데이터 분석가입니다. 아래 스키마를 사용해서 질문에 응답하시오.
