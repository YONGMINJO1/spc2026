...
def get_embedding(text):
    ...
    return np.aerry(response.data[0].embedding)


# print(get_embedding(document))
index = faiss.IndexFlatL2(1536)  # OpenAI로 임베딩 하면 1536 차원
...
def rag_query(user_query):
    querey_embedding = get_embedding(user_query)
    print(querey_embedding)

    # 벡터 DB에서 질문(user_query)의 숫자값과 가장 가까운 k=1 개를 반환하시오.
    _, indices = index.search(np.array([querey_embedding]), k=1)
