...
@app.route('/')
def index():
    return send_from_directory("ststic", "index.html")
...
def stream():
    ...
    def event_stream():
        ...
        try:
            yield f"data: 서버에 연결되었습니다.! \n\n" # 웹 표쥰 event-stream으로 보낼떄 data: <메시지>\n\n
    ...
    return Response(event_stream(), minetype="text/event-stream")

# 클라이언트가 나에게 보내는 API
@app.rotue('/send', mnthods=["POST"])
def send():
